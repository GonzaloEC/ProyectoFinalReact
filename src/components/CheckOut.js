...
import { createBuyOrder } from '../firebase/config.js';
...
function CheckOut() {
    const { cart, clearCart, totalProducts, totalPrice } = useCartContext();
      ...
      <div
        ...
        if (cart.length === 0) {
            ...
            <div className="row py-lg-5">
                <div className="col-12 carrito-">
                <p>No hay productos en su carrito</p>
                <p></p>
                <Link to="/" className='carrito-link'>Regresar al menú</Link>
              </div>
            </div>
          </section>
      } else if(OrderID) {
        return (
          <section id="carrito" className="py-2 text-center container slide-in-fwd-center">
            ...
            <div><Badge bg="info" className="m-1"><h6>Su ID de compra ${OrderID} Total de items: {totalProducts()}</h6></Badge><Badge className="m-3" bg="info"><h6> Costo Total: {totalPrice()} $</h6></Badge><div></div><div><Link to="/">regresar al catálogo</Link></div> 
              </div>
            </div>
          </section>
        )
  ...
  return (
    ...
    <main>
      <div className="py-3 text-center">
        <h2 className='title-title'>Checkout</h2>
        <p className="lead">Su lista de compra y formulario de pago.</p>
      </div>
      <div className="row">
        <div className="col-md-5 col-lg-4 order-md-last div__datos">
          <h4 className="d-flex justify-content-between align-items-center mb-3">
            <span className="title-title">Su carrito</span>
            <span className="badge span-total">{totalProducts()}</span>
          </h4>
          ...
          <li className="list-group-item d-flex justify-content-between lh-sm">
              <div>
                <h6 className="my-0">Total: $ {(totalPrice().toFixed(3))}</h6>
              </div>
            </li>
          </ul>
        </div>
        <div className="col-md-7 col-lg-8 div__datos">
          <h4 className="mb-3 title-title">Datos del cliente</h4>
        ...
        {successMessage()}
          ...
          <form className="needs-validation">
            ...
            <hr className="my-4" />
            <button onClick={handleSubmit} className="w-100 btn-pagar" type="submit">Pagar</button>
          </form>
